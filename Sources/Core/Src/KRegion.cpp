...
//----------------------------------------------------------------------
int KRegion::LoadObject(int nSubWorld, int nX, int nY)
{
	int npcs = 0;
	KPakFile	cData;
	char szFilePath[260], szFile[260];
	...
	if (cData.Open(szFile))
	{
		...
		//npcs += LoadServerNpc(nSubWorld, &cData, sElemFile[REGION_NPC_FILE_INDEX].uLength);
...
//----------------------------------------------------------------------
int	KRegion::LoadServerNpc(int nSubWorld, KPakFile *pFile, DWORD dwDataSize)
{
	...
	{
		memset(&sNpcCell,0,sizeof(sNpcCell));
		...
		{
			ZeroMemory(sNpcCell.szScript,sizeof(sNpcCell.szScript));
			//strcpy(sNpcCell.szScript,pMapListInfo->nNewWorldScript);
		}
...
{
	MessageBox(NULL,"Da vao day roi","KRegion::LoadClientObj",MB_OK);
...
#ifdef _SERVER
		if ((nCounter == m_nNpcSyncCounter >> 1) && (m_nNpcSyncCounter & 1))
		{
			// 发送同步信号
...
//	}
	...
	while(pNode)
	{
		pTmpNode = (KIndexNode *)pNode->GetNext();
#ifdef _SERVER
		if ((nCounter == m_nObjSyncCounter >> 1) && (m_nObjSyncCounter & 1))
		{
...
{
	KIndexNode *pNode = (KIndexNode *)m_ObjList.GetHead();
	while(pNode)
	{
