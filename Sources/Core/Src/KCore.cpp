...
BYTE	_declspec(dllexport) nCurSerIdx;
#else
IClient* g_pClient;
BOOL	 g_bPingReply;
#endif
...
//	g_SetFilePath("\\");
	...
	if (Weapon_PhysicsSkillIdFile.Load(WEAPON_PHYSICSSKILLFILE))
	{
		...
		for (int i = 0; i < nHeight; i ++)
		{
			...
			int nParticular  = 0;
			int nPhysicsSkill = 0;
			Weapon_PhysicsSkillIdFile.GetInteger(i + 2, 1, -1, &nDetail);
			Weapon_PhysicsSkillIdFile.GetInteger(i + 2, 2, -1, &nParticular);
			Weapon_PhysicsSkillIdFile.GetInteger(i + 2, 3, -1, &nPhysicsSkill);
