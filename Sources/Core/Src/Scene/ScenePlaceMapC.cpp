...
#ifndef _SERVER
BYTE*	KScenePlaceMapC::GetBarrierBuf(int nX, int nY)
{
	if (!m_ppbtBarrier)
		return NULL;

	int	nRegion  = nY * m_nMapWidth + nX;
		...
		if (m_bHavePicMap)
		{
			...
			if (m_EntireMapLTPosition.x == -1)
			{
...
#ifndef _SERVER
				// chi chay o client => size map

				m_nMapWidth = rc.right - rc.left + 1; 
				m_nMapHeight = rc.bottom - rc.top + 1;
				...
				for (y=0;y<m_nMapHeight;y++)
				{
					...
					{
						KRegion::LoadLittleMapData(x+rc.left,y+rc.top,(char*)pszScenePlaceRootPath,GetBarrierBuf(x,y));
					}
				}
