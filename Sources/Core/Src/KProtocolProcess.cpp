...
{	
	...
	if (nRegion == -1)
	{
		...
		int nRegionX = pSync->m_dwMapX / (defLOGIC_CELL_WIDTH * defLOGIC_REGION_WIDTH);
		int nRegionY = pSync->m_dwMapY / (defLOGIC_CELL_HEIGHT * defLOGIC_REGION_HEIGHT);
...
void	KProtocolProcess::PlayerBlockItemReturn(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		...
		Player[CLIENT_PLAYER_INDEX].m_ItemList.UnlockOperation();
			...
			sMsg.byPriority = 1;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage,"%s%s%s", "Х nh b秓 hi觤 trang b?", Item[nIdx].GetName()," !");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void	KProtocolProcess::PlayerUnBlockItemReturn(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		 ...
		 Player[CLIENT_PLAYER_INDEX].m_ItemList.UnlockOperation();
			...
			sMsg.byPriority = 1;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage,"%s%s%s", "Х x竎 nh薾 th竜 b秓 hi觤 trang b?", Item[nIdx].GetName()," !");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void	KProtocolProcess::s2cAutoPlaySync(BYTE* pMsg)
{
	...
	if (btAuto == 1)
	{
		if (bActive)
		{
			...
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage, "Х b藅 auto!");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
		}
		else
		{
			...
			sMsg.byPriority = 0;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage, "T総 auto!");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);		
		}
...
void KProtocolProcess::OpenShopGold(int nIndex, BYTE* pProtocol)
{
	...
	if (nIndex > 0 && nIndex < MAX_PLAYER)
	{
	    ...
	    //KPlayerChat::SendSystemInfo(1, nIndex, MESSAGE_SYSTEM_ANNOUCE_HEAD, "Hi謓 t筰 kh玭g th?s?d鬾g. K?tr﹏ c竎 s?m?v祇 9 gi?ng祔 25/1/2013 !", strlen("Hi謓 t筰 kh玭g th?s?d鬾g. K?tr﹏ c竎 s?m?v祇 9 gi?ng祔 25/1/2013 !") );
