...
bool CoreServerShell::IsMap(DWORD id)
{
	return g_SubWorldSet.SearchWorld(id) >= 0;
}
...
int CoreServerShell::AddCharacter(int nExtPoint, int nChangeExtPoint, void* pBuffer, GUID* pGuid)
{
	...
	if (pData && pData->BaseInfo.szName[0])
	{
		...
		if (Player[nIdx].m_byLixian == LIXIAN_IDLE)
		{
...
bool CoreServerShell::PlayerDbLoading(int nPlayerIndex, int bSyncEnd, int& nStep, unsigned int& nParam)
{
	...
	else if (Player[nPlayerIndex].m_byLixian == LIXIAN_DOING || pData)	
	{
...
{
	if (Player[nIndex].m_byLixian == LIXIAN_DOING)
	{
		Player[nIndex].m_bIsQuiting = false;
		Player[nIndex].m_byLixian = LIXIAN_IDLE;
		
		int x,y;
		int map = SubWorld[Npc[Player[nIndex].m_nIndex].m_SubWorldIndex].m_SubWorldID;
		Npc[Player[nIndex].m_nIndex].GetMpsPos(&x, &y);		

		Npc[Player[nIndex].m_nIndex].ChangeWorld(map, x, y, true);	
	}
	else
	{
...
BOOL CoreServerShell::GroupChat(IClient* pClient, DWORD FromIP, unsigned long FromRelayID, DWORD channid, BYTE tgtcls, DWORD tgtid, const void* pData, size_t size)
{
	...
	case tgtcls_team:
		...
		int nTargetIdx = g_Team[tgtid].m_nCaptain;
