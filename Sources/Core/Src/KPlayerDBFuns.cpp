...
int KPlayer::LoadDBPlayerInfo(BYTE *pPlayerInfo, int &nStep, unsigned int &nParam)
{
	...
	{
	case STEP_BASE_INFO:
		if (m_byLixian == LIXIAN_DOING)//dang uy thac
		{
...
label_retry:
	...
	int nSubWorld = g_SubWorldSet.SearchWorld(tempPos.m_nSubWorldID);
	if (nSubWorld == -1)
	{
		printf("Khong ton tai map [%d] trong GS %d\n",tempPos.m_nSubWorldID,nCurSerIdx);
		return -1;
	}

	m_nIndex = NpcSet.Add(nSex, nSubWorld,
		tempPos.m_nMpsX,
		tempPos.m_nMpsY);
...
//	else 
	...
	{
		printf("Exception Have Caught When Execute Script[\\script\\admin\\accloi.lua]!!!!!");
	}
