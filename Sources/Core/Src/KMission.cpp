...
BOOL	KMission::ExecuteScript(DWORD dwScriptId,  char * szFunName, int nParam)
{
	...
	{
		printf("Exception KMission::ExecuteScript");
		printf("Exception KMission::ExecuteScript[%d (%s) => %s]", dwScriptId, szFunName, ex.what());
		//g_DebugLog("Exception Have Caught When Execute Script[%d,%s]!!!!!",dwScriptId,szFunName);
		return FALSE;
	}
	return TRUE;
}
