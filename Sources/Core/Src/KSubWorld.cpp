...
//	_ASSERT(nR >= 0 && nR < 9);
	...
	x += nX * defLOGIC_CELL_WIDTH;
	y += nY * defLOGIC_CELL_HEIGHT;
...
{
	int x = Rx / (defLOGIC_REGION_WIDTH * defLOGIC_CELL_WIDTH);
	int	y = Ry / (defLOGIC_REGION_HEIGHT * defLOGIC_CELL_HEIGHT);
	...
	*nX = x / defLOGIC_CELL_WIDTH;
	*nY = y / defLOGIC_CELL_HEIGHT;

	*nDx = (x - *nX * defLOGIC_CELL_WIDTH) << 10;
	*nDy = (y - *nY * defLOGIC_CELL_HEIGHT) << 10;
}

BYTE	KSubWorld::TestBarrier(int nMpsX, int nMpsY)
{
	int x = nMpsX / (defLOGIC_REGION_WIDTH * defLOGIC_CELL_WIDTH);
	int	y = nMpsY / (defLOGIC_REGION_HEIGHT * defLOGIC_CELL_HEIGHT);
#ifdef _SERVER
	// 非法的坐标
	if (x >= m_nWorldRegionWidth + m_nRegionBeginX || y >= m_nWorldRegionHeight + m_nRegionBeginY || x < m_nRegionBeginX || y < m_nRegionBeginY)
		return 0xff;
	...
	int nCellX = x / defLOGIC_CELL_WIDTH;
	int nCellY = y / defLOGIC_CELL_HEIGHT;

	int nOffX = x - nCellX * defLOGIC_CELL_WIDTH;
	int nOffY = y - nCellY * defLOGIC_CELL_HEIGHT;
...
BYTE KSubWorld::TestBarrier(int nRegion, int nMapX, int nMapY, int nDx, int nDy, int nChangeX, int nChangeY)
{
	...
	if (nDx < 0)
	{
		nDx += (defLOGIC_CELL_WIDTH << 10);
		nMapX--;
	}
	else if (nDx >= (defLOGIC_CELL_WIDTH << 10))
	{
		nDx -= (defLOGIC_CELL_WIDTH << 10);
		nMapX++;
	}
	...
	if (nDy < 0)
	{
		nDy += (defLOGIC_CELL_HEIGHT << 10);
		nMapY--;
	}
	else if (nDy >= (defLOGIC_CELL_HEIGHT << 10))
	{
		nDy -= (defLOGIC_CELL_HEIGHT << 10);
		nMapY++;
	}
	...
	if (nMapX < 0)
	{
		...
		nMapX += defLOGIC_REGION_WIDTH;
	}
	else if (nMapX >= defLOGIC_REGION_WIDTH)
	{
	...
	if (nMapY < 0)
	{
		...
		nMapY += defLOGIC_REGION_HEIGHT;
	}
	else if (nMapY >= defLOGIC_REGION_HEIGHT)
	{
...
//	m_nRegionHeight = 1024 / 32;
	...
	m_nRegionBeginX = (int)sRect.left;
	m_nRegionBeginY = (int)sRect.top;
	int m_nRegionEndX   = (int)sRect.right;
	int m_nRegionEndY   = (int)sRect.bottom;
...
//	m_nCellHeight = defLOGIC_CELL_HEIGHT;
	...
	for (nY = 0; nY < m_nWorldRegionHeight; nY++)
	{
		for (nX = 0; nX < m_nWorldRegionWidth; nX++)
		{
			...
			{
				m_Region[nIdx].Init(defLOGIC_REGION_WIDTH, defLOGIC_REGION_HEIGHT);
	...
	for (nY = 0; nY < m_nWorldRegionHeight; nY++)
	{
		...
		{
			g_SetFilePath(szPath);
			npcs += m_Region[nY * m_nWorldRegionWidth + nX].LoadObject(m_nIndex,nX + m_nRegionBeginX, nY + m_nRegionBeginY);
		}
	}
	...
	for (int i = 0; i < 8; i++)
	{
		...
		if (nConIdx < 0)
		{
			...
			if (m_Region[nConIdx].Load(nX + nXOff[i], nY + nYOff[i]))
			{
				...
				m_Region[nConIdx].Init(defLOGIC_REGION_WIDTH, defLOGIC_REGION_HEIGHT);
