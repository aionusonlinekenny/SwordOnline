...
int KNpcSet::GetDistance(int nIdx1, int nIdx2)
{
	...
	{
		int XOff = (Npc[nIdx1].m_MapX - Npc[nIdx2].m_MapX) * defLOGIC_CELL_WIDTH;
		XOff += (Npc[nIdx1].m_OffX - Npc[nIdx2].m_OffX) >> 10;

		int YOff = (Npc[nIdx1].m_MapY - Npc[nIdx2].m_MapY) * defLOGIC_CELL_HEIGHT;
...
//-------------------------------------------------------------------------
void KNpcSet::GetLife(int dw, int& img, int& cur, int& max)
{
	int nIdx = 0;
	while (1)
	{
		...
		if (Npc[nIdx].m_dwID == dw)
		{
			img = Npc[nIdx].m_ImagePlayer;
			
			cur = Npc[nIdx].m_CurrentLife;
			max = Npc[nIdx].m_CurrentLifeMax;
			break;
		}
	}
}
