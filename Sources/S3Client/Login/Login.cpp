...
#define VLTK_WINCLASS "VoLamTruyenKy Class"
int ChildCount;
BOOL CALLBACK enumWindowCallback(HWND hWnd, LPARAM lparam) 
{
    ...
    GetClassNameA(hWnd, buffer, 63);
    
    if (strcmp(buffer, VLTK_WINCLASS) == 0) {
        ++ChildCount;
    }
...
int KLogin::Request(const char* pszAccount, const KSG_PASSWORD* pcPassword, int nAction)
{
	...
	if (ChildCount >= 9)
	{
		MessageBox(NULL,"Qua nhieu cua so","Error",MB_OK);
		return 0;
	}	
