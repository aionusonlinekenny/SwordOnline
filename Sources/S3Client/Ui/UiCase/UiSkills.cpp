...
#define 	SCHEME_INI_LIVE				"UISkillsLive.ini"
#define 	SCHEME_INI_FIGHT			"UISkillsFight.ini"
...
unsigned int sk_drag = 0;
int	KUiFightSkillSubPage::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	if (sk_drag)
	{
		if ((uMsg == WM_LBUTTONDOWN)||(uMsg == WM_LBUTTONUP))//nhan
		{
			MessageBox(NULL,"Da clc chuot","KUiFightSkillSubPage",MB_OK);

			int	x = LOWORD(nParam);
			int y = HIWORD(nParam);

			KUiPlayerBar::GetIfVisible()->DropSkill(x,y);
		}
		else
		{
			if (GetKeyState(VK_CONTROL) & 0x8000 || (uParam & MK_SHIFT))
				return 0;
		}
	}
	if (uMsg == WND_N_LEFT_CLICK_ITEM && uParam)
	{
		...
		if (pObj->uGenre != CGOG_NOTHING)
		{
			if (pObj->Dragging)
			{
				...
				Wnd_DragBegin(pObj, DrawDraggingGameObjFunc);
			}
			else
...
/*
	if (sk_drag)
	{
		if ((uMsg == WM_LBUTTONDOWN)||(uMsg == WM_LBUTTONUP))//nhan
		{
			...
			KUiPlayerBar::GetIfVisible()->DropSkill(x,y,m_clone);
		}
		else
		{
			if (GetKeyState(VK_CONTROL) & 0x8000 || (uParam & MK_SHIFT))
