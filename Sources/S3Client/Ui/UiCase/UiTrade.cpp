...
#define	STR_WAIT_TRADING		"Ch?x竎 nh薾"
#define	STR_OTHER_OK			"цi phng  kh鉧 giao d辌h"
#define	STR_OTHER_NOT_OK		"Ch?i phng kh鉧 giao d辌h"
...
void KUiTrade::OnAdjustMoney(bool bAdd)
{
	...
	int luong1 = m_nTakewithMoney%10000;
		char ngan_luong1[32] = "";
	if (van1 > 0)
	sprintf(ngan_luong1,"%d %s ",van1,"v筺");

	sprintf(ngan_luong1,"%s%d %s",ngan_luong1,luong1,"lng");
