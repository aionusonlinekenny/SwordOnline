...
void CGatewayClient::Proc0_NotifySvrIP(const void* pData, size_t size)
{
	...
	if (pN->dwSvrIP != 0)
	{
		CNetConnectDup conndup = g_HostServer.FindHostConnectByIP(CHostConnect::FindHostConnectByidx(pN->dwSvrIP));
		if (conndup.IsValid())
		{//relay
	...
	if (!passed)
	{//lose
		CNetConnectDup conndup2 = g_HostServer.FindHostConnectByIP(CHostConnect::FindHostConnectByidx(rAskWayMap.fromIP));
		if (conndup2.IsValid())
		{
			size_t sizeLoseData = sizeof(RELAY_DATA) + sizePckg;
...
BOOL CGatewayClient::PassAskWayMap(DWORD fromIP, const RELAY_ASKWAY_DATA* pAskWayMap)
{
	...
	if (IsReady())
	{
		if (m_mapAskWayMaps.size() < MAX_PACKAGECOUNT)
		{
			...
			rTRACE("----PassAskWayMap A (%08X)----",req.dwMapID);
