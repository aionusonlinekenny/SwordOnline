...
BOOL CChannelMgr::SomeoneChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_SOMEONECHAT_CMD* pSomeoneChatCmd)
{
	...
	if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(pSomeoneChatCmd->someone), &hostconndup, NULL, &nameid, &param))
	{
		...
		if (PassToSpecMan(hostconndup.GetID(), param, nameid, pCscSync, syncsize))
			sent = TRUE;
	}
...
BOOL CChannelMgr::ChannelChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_CHANNELCHAT_CMD* pChannelChatCmd)
{
	...
	CHANNINFO& channInfo = (*itChannid).second;

	if (pChannelChatCmd->cost != channInfo.cost)
		return FALSE;
	...
	{
		std::string channels = channInfo.channname;
		if (channels[0] != m_chChannEsc) // fix chu Tau
			channels = "\\W";

		if (channels[0] == m_chChannEsc)
		{//relegate to gamesvr

			DWORD channIP = 0, theID = -1;

			if (channels[1] != 'W')
			{
				theID = ParseChannelName(channels, &channIP);
			...
			if (channels[1] == m_chChannFAC)// mon phai
			{
				if (theID == -1)
					return FALSE;
			...
			else if (channels[1] == m_chChannTONG)// bang hoi
			{
				if (theID == -1)
					return FALSE;
			...
			else if (channels[1] == m_chChannTEAM)// to doi
			{
				if (theID == -1 || channIP == 0)
					return FALSE;

				CNetConnectDup connDup = g_ChatServer.FindChatConnectByIP(srcIP);//channIP
				if (connDup.IsValid())
				{
			...
			else if (channels[1] == m_chChannKIM)
			{
				if (theID == -1 || channIP == 0)
					return FALSE;
			...
			else if (channels[1] == m_chChannSCRN
				|| channels[1] == m_chChannBC)
			{
//				if (channIP == 0)
//					return FALSE;
...
//				assert(channIP != 0 && theID == 0);
				...
				if (connDup.IsValid())
				{
					BOOL isScrn = channels[1] == m_chChannSCRN;
		...
		{//broad on channel
			...
			for (IP2PLAYERSMAP::iterator itIP = channInfo.mapIp2Players.begin(); itIP != channInfo.mapIp2Players.end(); itIP++)
			{
				DWORD IP = (*itIP).first;
				PLAYERSMAP& rPlayers = (*itIP).second;
				...
				if (!mans.empty())
				{
					...
					PassToGroupMan(srcIP, FALSE, mans, pCccSync, syncsize);
				}
...
BOOL CChannelMgr::Block_QueryChannelID(const std::_tstring& channel, DWORD ip, unsigned long param, DWORD nameid, DWORD serverip)
{
	std::_tstring channname = MakeChannelName(channel, serverip); //MakeChannelName(channel, ip);
	if (channname.empty())
		return FALSE;
...
BOOL CChannelMgr::B_QueryChannelID(const std::_tstring& channel, DWORD ip, unsigned long param, DWORD nameid, DWORD serverip)
{
	return Block_QueryChannelID(channel, ip, param, nameid, serverip);
}
...
*/
//Bang Hoi
BOOL CChannelMgr::SayOnChannel(DWORD channid, const std::_tstring& name, const std::_tstring& sent)
{
	DUMMY_AUTOLOCKREAD(m_lockChannel);
