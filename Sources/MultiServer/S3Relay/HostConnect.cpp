...
UINT CHostConnect::FindHostConnectByidx(unsigned long param)
{
	if (m_sHostServer.count(param))
		return m_sHostServer[param];
	else
		return 0;
}
...
void CHostConnect::RecvPackage(const void* pData, size_t size)
{
	...
	else if (pHeader->ProtocolFamily == pf_updateserver)//update noi bo
	{
...
void CHostConnect::Proc0_ServerCommunity(const void* pData, size_t size)
{
    ...
    if (pHeader->ProtocolID == relay_s2c_udateserveridx)
	{//
		tagLeaveGame2 * pServerData = (tagLeaveGame2*)pData;
		m_sHostServer[pServerData->nSelServer] = GetID();//?? ?? ?????????? ??GS ?????
		rTRACE("--- GS (%d) map by %08X----",pServerData->nSelServer,m_sHostServer[pServerData->nSelServer]);
	}
}
...
void CHostConnect::Proc1_Relay_Data(const void* pData, size_t size)
{
	...
	{
		if (g_HostServer.IsConnectReady(m_sHostServer[pRelayData->nToIP]))//pRelayData->nToRelayID
		{
			CNetConnectDup conndup;
			if (pRelayData->nToRelayID==0)
				conndup= g_HostServer.FindNetConnect(pRelayData->nFromIP);//pRelayData->nToRelayID
			else
				conndup= g_HostServer.FindNetConnect(m_sHostServer[pRelayData->nToRelayID]);//????????
		...
		{
			rTRACE("---Server (%d) not found, replay fail---",pRelayData->nToIP);
		}		
/*
...
void CHostConnect::Proc2_Relay_AskWay_MapID(const void* pData, size_t size)
{
	...
	g_GatewayClient.PassAskWayMap(pAskWayData->nFromIP, pAskWayData);
}
...
void CHostConnect::Proc1_PlayerCommunity_QueryChannelID(const void* pData, size_t size)
{
	...
	g_ChannelMgr.B_QueryChannelID(pPlayerCommQID->channel, GetID(), pSrcInfo->lnID, pSrcInfo->nameid, GetIP());
}
