...
UINT CGamePlayer::WaitForAccPwd()
{
	...
	cprintf( "CGamePlayer::WaitForAccPwd [AccountName:%s]\n", pAccName );
        ...
        if (pLAI)
        {
            ...
            if (nCheckProtocolVersion && nSeri == 192168001)
            {
                // 如果协议版本相同，继续判断账号
                if (pAccName[0]) 
		        {
...
bool CGamePlayer::_SyncRoleInfo_ToGameServer( const void *pData, size_t dataLength )
{
	...
	if(strcmp(m_sAccountName.c_str(), pRoleData->BaseInfo.caccname) == 0)
	{
		...
		{
			sprintf(info,"Server for map %d not found",pRoleData->BaseInfo.irevivalid);
			pGServer = CGameServer::QueryServer( pRoleData->BaseInfo.irevivalid );
		}
		else
		{
			sprintf(info,"Server for map %d not found",pRoleData->BaseInfo.ientergameid);
		...
		{
			MessageBox(NULL,info,m_sAccountName.c_str(),MB_OK);
		}
	}
